...
import random
import os
from level_prestige import dhaven
from level_prestige import Starting
from level_prestige import Cleric
from level_prestige import Support
from tom import Tom
...
class ROD(dhaven, Gnome, Sunless, Starting, Cleric, Coral, Art, Toz, Mithril, Support, Tom, Canyon, Winter, Tree, King, Shire):
    ...
    def __init__(self, user, pw, container, function, support = None,  take_input_bool = False):
        ...
        self.weapons = {
                        ...
                        "Shock Whip":"whip",
                        'black hilted sword "Shieldbreaker"': "shieldb",
                        "katana":"katana",
                        "amazon spear":"spear"
        ...
        try:
            self.alt_info = pickle.load(open("alts/info_%s.pckle"%self.name,'r'))
        except:
            self.alt_info = {}
        ...
        if True:
            ...
            if self.charclass in ["Vampire","Dread"]:
            ...
            for skill, v in self.slist:
                if skill in ["kick","cuff","spurn","surestrike","lunge",'shieldbash']:
                    self.attack = skill
            ...
            if "\n" in self.bufln:
                ...
                for ln in self.bufln.split("\n"):
                    ...
                    if "Your stomach cannot contain any more." in ln:
                        ...
                        if int(self.HP) < int(self.MAXHP)*0.45:
                            self.rod.write("flee\nquit\n")
                            self.status = "restart"
                        if cspring == False:
                            makespring = True
        ...
        if self.lag == 0:
            ...
            if int(self.HP) < int(self.MAXHP)*0.8:
                ...
                if self.disarm:
                    ...
                    # Priority 1: ALWAYS prioritize healing over mana (handled above)
                    
                    # Priority 2: Handle emergency mana situation
                    if needs_emergency_mana and spell != None and self.charclass in ['Mage',"Augurer", "Nephandi", "Cleric","Harbinger"]:
                        self.printc("EMERGENCY: Low mana (%d MP) for spell, quaffing mana potion!" % int(self.MP), 'red')
                        self.manaup()  # Force mana quaff for emergency situations
                    
                    # Priority 3: Cast spell if we have enough mana, otherwise use physical attack
                    if int(self.MP) >= min_mana_needed or (int(self.MP) > 8 and (self.charclass == "Vampire" or self.charclass == "Dread")):
                        ...
                        if spell != None and self.charclass in ['Mage',"Augurer", "Nephandi", "Cleric","Harbinger"]:
                            # Intelligent mana quaffing based on class and spell costs
                            quaff_chance = 0.9  # High chance for reliable mana
                            if self.charclass in ["Fathomer","Harbinger"]:
                                quaff_chance = 0.4  # Lower for fathomers and harbingers
                            ...
                            if self.random.random() < quaff_chance:
                                self.manaup()
                                self.printc("Quaffing mana - MP: %d/%d" % (int(self.MP), int(self.MAXMP)), 'cyan')
    ...
    def find_attack_main(self):
        ...
        if self.charclass == "Mage":
            ...
            if len(spells) > 0:
                ...
                return None
            return "cast '%s'"%spell
        elif self.charclass in ["Vampire","Dread"]:
            for x in self.slist:
            ...
            if len(spells) >= 1:
                ...
                return None
            if spell == "decaying touch":
                return 'decay'
            return "cast '%s'"%spell

        elif self.charclass == "Harbinger":
            for x in self.slist:
    ...
    def checkstock(self):
        ...
        print self.containers[self.container]

        try: heals = self.containers[self.container]["Dragons' lament"]
        except: heals = 0
        try: sancs = self.containers[self.container]["a sanctuary potion"]
        except: sancs = 0
        try: manas = self.containers[self.container]['a potion of black orchid petals']
        except: manas = 0
        try: nips = self.containers[self.container]['a nip of Nooracht']
        except: nips = 0
        ...
        self.printc("heals (%d) manas (%d) nips (%d) sancs (%d)"%(self.heals, self.manas, self.nips, self.sancs),'gold')

    def restock(self):
        self.get_loc()
        if self.location != "Hall of Honor":
            if self.charclass == "Cleric" and self.level == 50:
                self.waitcmd("c word\nd\nd\nn")
            else:
                self.waitcmd("rec scroll\nget recall %s\nd\nd\nn"%self.container)
        else:
            self.checkstock()
            self.go("w")

            self.waitcmd("fill %s %d sanctuary shelf"%(self.container, 5-self.sancs))
            if self.level >= 8:
                self.waitcmd("fill %s %d lament shelf"%(self.container, 100-max([self.heals,0])))
            else:
                self.waitcmd("fill %s %d lament shelf"%(self.container, 10-max([self.heals,0])))
            #self.waitcmd("fill %s %d orchid shelf"%(self.container, 100-self.manas))
            self.go("e")
        ...
        if loc == "dhsquare":
            if self.location == "Thoth's Rune on Vertic Avenue":
                ...
                if self.charclass == "Barbarian":
                    ...
                    self.time.sleep(2.5)
                elif self.name in ["Dasani", "Zazival"]:
                    if len(self.inv) >= 5:                                                                                                                                                                                   
                        #self.rod.write("wear %s\nwear %s\ndrop all\nget pipe\nget tinder\nwear all\nget %s\nget recall\n"%(self.weaponkey,self.weaponkey,self.container))
                        self.rod.write("wear %s\nwear %s\nwear all\nget %s\nget recall\n"%(self.weaponkey,self.weaponkey,self.container))
                    printc("RECALLING %s located %s"%(self.name,self.location))
                    self.waitcmd("rec scroll\nget recall %s"%self.container)
                    self.go("d;d;n")
                    self.checkstock()
                    self.time.sleep(1)
                    if (self.heals < 10 or self.sancs < 5) and self.level < 8:
                        self.restock()
                    elif self.heals < 90 or self.sancs < 5: #self.manas < 100
                        self.restock()
                    
                    self.go("e")
                    self.waitcmd("say town\ns\n")
                else:
                    if self.charclass == "Cleric" and self.level == 50:
                        printc("RECALLING CLERIC %s located %s"%(self.name,self.location))
                        self.waitcmd("c word")
                        self.go("d;d;n")
                        if self.manas < 80 or self.heals < 70 or self.sancs < 5:
                            self.restock()
                        self.go("e")
                        self.waitcmd("say town\ns\n")
                    else:
            ...
            if self.location == "Thoth's Rune on Vertic Avenue":
                ...
                if self.charclass == "Barbarian":
                    ...
                    self.rod.write("buy recall\nput recall my.chest\ne\nn\nn\nsay I wish to visit the city dwellers\n")
                    self.time.sleep(2.5)
                elif self.name in ["Dasani"]:
                    printc("RECALLING %s located %s"%(self.name,self.location))
                    self.rod.waitcmd("rec scroll\nget recall %s"%self.container)
                    self.go("d;d;n")
                    self.restock()
                    self.go("e")
                    self.cmdwait("say town\n")
                else:
                    if self.charclass == "Cleric" and self.level == 50:
                        printc("RECALLING CLERIC %s located %s"%(self.name,self.location))
                        self.rod.waitcmd("c word")
                        self.go("d;d;n")
                        self.restock()
                        self.go("e")
                        self.cmdwait("say town\n")
    ...
    def read(self):
        ...
        for i in range(len(rln)):            
            ...
            if "You try to smoke a Ho'chran pipe, but it's not lit." in rln[i]:
                self.pipelit = False
            elif "You carefully light a Ho'chran pipe." in rln[i]:
                self.pipelit = True
                self.pipeempty = False
            elif "You try to light a Ho'chran pipe, but it's empty." in rln[i]:
                self.pipeempty = True
    ...
    def buyblue(self):
        ...
        if self.charclass == "Fathomer":
            ...
            r = ''
            self.waitcmd("give 800k coin salamon\nvis")

            r = tmp.waitcmd("buy 40 blue\ngive bag %s"%self.name)
            self.printc(r,"red")
            self.waitcmd("empty bag %s\ndrop bag\n"%self.container)
    ...
    def emergency_quit(self):
        ...
        self.time.sleep(6)
        msg = self.random.choice(["hey, gtg after this"])
