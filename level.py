...
class dhaven:
    ...
    def func_dhaven(self):
        ...
        if self.phase == 0:
            ...
            if "curse" in self.aff and self.charclass != "Barbarian":
                if self.clericon:
                    ...
                    try: self.cleric.rod.write("cast \"remove curse\" %s\n"%self.name)
                    except:
            ...
            if upgear and self.phase == 1:
                ...
                canreplace = []
                for wearloc, eq in list(self.eq.items()):
                    for e in eq:
                        if e in replace:
                            E, R = e, replace[e]
        ...
        if self.phase == 1:
            ...
            getpotionmana = False
            if self.potnamemana in self.containers[self.container]:
                if self.containers[self.container][self.potnamemana] < 10:
                    getpotionmana = True
            else:
                if self.potnamemana != None:
                    getpotionmana = True

            if (getpotion or getpotionmana) and self.phase == 2:
            ...
            #if getsanc and self.phase == 2:
            #    sys.stdout.write("GETTING SANC...\n")
            #    self.status_msg = "Getting sanctuary..."
            #    self.godh()
            #    self.go("ne")
            #    #self.rod.write("say heal\nsay sanc\nsay elem\nsay refresh\n")
            #    self.rod.write("say heal\nsay sanc\nelementals\nsay refresh\n")
            #    sys.stdout.write("waiting for sanc spells 30s...\n")
            #    time.sleep(30)
            #    self.check_affect()
            #    self.go("sw")

                # if sanc still not there, then get cleric to cast on us
        ...
        if self.phase >= 2:
            ...
            if self.support != None or self.master != None:
                if self.master == None: # this is the master
                    self.ready = True
                    T = 0
                    self.godh()
                    success = False
                    while True:
                        print('\n',self.name, "waiting for parity check with", self.support.name)
                        self.printc("Waiting support "  + self.support.name + " Phase" + str(self.support.phase))
                        self.status_msg = "Waiting for support"
                        
                        T += 1
                        self.time.sleep(2)
                        
                        L = self.rod.read_very_eager()
                        if self.support.location == "Darkhaven Square" and self.support.ready:
                            self.support.rod.write("vis\nfollow %s\nver\n"%(self.name))
                        
                            r = ''
                            while True:
                                rl = self.support.rod.read_very_eager()
                                r += rl
                                if "SMAUG 2.6" in r:
                                    if ("You now follow" in r) and T < 20:
                                        self.rod.write("group all\n")
                                        self.support.following = True
                                        success = True
                                        break
                                    elif T >= 20:
                                        self.ready = False
                                        self.phase = 1
                                        self.printc("Lead waited 20 seconds for parity... restarting phase 1...")    
                                    break
                        elif T >= 20:
                            self.ready = False
                            self.phase = 1
                ...
                if self.master != None and self.phase == 2:
                    ...
                    self.godh()
                    self.whereami()
                    while self.location != "Darkhaven Square":
                        self.godh()
                        self.time.sleep(10)
                    ...
                    while True:
                        ...
                        self.printc("\nSUPPORT: %s is ready to go as the support"%self.name)
                        self.status_msg = "Waiting for lead"
                        T += 1
                        if T >= 20 and not self.following:
                            self.phase = 1
                            self.printc("Support waited 20 seconds for parity... restarting phase 1...")
                            break
                        elif self.following:
                            return "support"
                    self.ready = False

            print(self.master, self.phase)
            if self.master == None and self.phase == 2:
                if not self.clericon:
                    self.log_cleric()
                ...
                if len(allkills) > 0:
                    for x in list(allkills.items()):
                        self.printc("Killed %s %d times"%(x[0],x[1]))
                        if ((x[1] >= 6 and self.level >= 26 and x[0] != "A twisted bronze statue" and x[0] != 'A tw' and x[0] != 'The crab guard') or 
...
if __name__ == "__main__":
    ...
    # This file should be imported by connect.py, not run directly
    # Use: python connect.py <character_name> <cleric_name>
    # Example: python connect.py Daltin Kaeval
    
    print("This file should not be run directly.")
    print("Use: python connect.py <character_name> <cleric_name>")
    print("Example: python connect.py Daltin Kaeval")
    sys.exit(1)
