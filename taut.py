...
class dhaven:
    ...
    def func_dhaven(self):
        ...
        self.go("#8 s;w")
        self.get_loc()
        if self.location == "Outside a Mysterious Inn":
            
            self.printc("%s is ready to roll!"%(self.name),'gold')
            self.check_cont("basket")
    ...
    def func_bl(self):
        ...
        if self.location == "The Fabric Shop":
            #initialize sequence
            self.go("w;s;s;w")

        elif self.location == "The Street to Taut":
            self.go("w")
            ...
            if "\n" in self.bufln:
                ...
                for ln in self.bufln.split("\n"):
                    ...
                    if "Drink what?" in ln and makespring:
                        ...
                        if self.charclass != "Barbarian":
                            cspring = True
                            print("get icicle basket\nremove %s\nwear icicle\nbrandish\nremove icicle\nput icicle basket\nwear %s\ndr\n"%(self.weaponkey, self.weaponkey))
                            self.rod.write("get icicle basket\nremove %s\nwear icicle\nbrandish\nremove icicle\nput icicle basket\nwear %s\ndr\n"%(self.weaponkey,self.weaponkey))
    ...
    def read(self):
        ...
        for i in range(len(rln)):            
            ...
            if "starved" in rln[i] or "You are mite peckish" in rln[i] or "You are STARVING!" in rln[i]:
                self.rod.write("eat turkey basket\n")
...
if __name__ == "__main__":
    ...
    user, pw, container = "Bygnok", "1q2w3e4r", "basket"
    ...
    while not allready:
        allready = True
        for user in alts:
            users[user] = ROD(user,pw,container,'dhaven')
            
        for user in alts:
            if not users[user].ready:
                allready = False
        time.sleep(10)
